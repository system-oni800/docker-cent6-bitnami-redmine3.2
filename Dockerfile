...
ENV PW adminpass00
ENV ZABBIX_SVR _SET_ZABBIX_SERVER_ADDRESS_WITH_INTERNAL_IP_
# I always set this = 172.17.0.1
# ENV http_proxy="_SET_YOUR_PRXY_ADDRESS_" if you need..
# Install bitnami-redmine 3.2.0-0

RUN yum update -y -q
RUN yum install -y -q sudo tar passwd expect spawn patch yum-cron logrotate
